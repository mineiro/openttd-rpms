...
### Bundled fonts: additional OFL notice

The four installed `media/baseset/OpenTTD-{Mono,Sans,Serif,Small}.ttf` files carry **SIL Open Font License 1.1** declarations in their TrueType `name` tables: name ID 0 credits `Richard Wheeler (Zephyris)`, ID 13 contains the complete OFL 1.1, and ID 14 points to `http://scripts.sil.org/OFL`. FontTools inspection of the actual 16.0-beta2 archive confirmed that all four contain identical license text. Thus add **OFL-1.1** to the package's aggregate `License` expression and preserve that font-specific notice, preferably by exporting the embedded copyright and license to a readable `%license` file. The notice permits bundled redistribution and accepts accessible metadata, but a standalone notice makes it easier to inspect.

There is an upstream documentation discrepancy: [OpenTTD-font.md](https://github.com/OpenTTD/OpenTTD/blob/16.0-beta2/media/baseset/OpenTTD-font.md) describes font release 0.7 as GPL-2.0 and [the font repository's root LICENSE](https://github.com/OpenTTD/OpenTTD-TTF/blob/0.7/LICENSE) is GPLv2. The [font tag's actual Sans TTF](https://github.com/OpenTTD/OpenTTD-TTF/blob/0.7/openttd-sans/OpenTTD-Sans.ttf) also embeds the OFL declaration. Preserve the GPL notice and font provenance as well; do not erase the explicit per-font OFL declaration or describe this as confirmed dual licensing. This is separate font-asset licensing, not a change to the game code's GPL terms. [Font release 0.7](https://github.com/OpenTTD/OpenTTD-TTF/releases/tag/0.7) provides compiled assets; the tagged repository also contains editable `.sfd` sources.
